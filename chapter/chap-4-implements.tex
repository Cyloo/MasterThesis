...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{cut-sidebar}
  \end{minipage}
  }
  \subfloat[]{
...
我们注意到，在病历图片中，虽然字段间有一定的间隔，但字与字之间，甚至是字的内部，很多都是不连通的，如果我们直接对\autoref{pic:bingli1_contours_origin}进行连通域划分，效果会很差，因此，需要对原图片进行与前一小节类似的灰度化、二值化、腐蚀等处理，将字内部、字与字之间都连接起来，同时保持字段的分隔，\autoref{bingli1_contours_origin}经过这一系列转换，我们得到如\autoref{pic:bingli1_contours_eroded}所示的二值化腐蚀图像，然后对它进行连通域分析，再将找到的连通域位置映射到原图，就得到\autoref{pic:bingli1_contours}，我们看到，各个字段被矩形框框住，同时字段与字段之间很好的分隔开了，字段提取工作完成。
